...
class TestSample:
    ...
    def test_reading(self):
        ...
        Test File Reading
        """
        assert callable(getattr(ssar,'csvToDictOfLists')) , "csvToDictOfLists function not defined"
        assert isinstance(ssar.csvToDictOfLists("pgh.csv"),dict) , "csvToDictOfLists should return a dictionary"
        assert len(ssar.csvToDictOfLists("pgh.csv")) == len(self.pgh) , "Lengths do not match. Check you read all the records and removed the header."
        assert ssar.csvToDictOfLists("pgh.csv") == self.pgh , "Make sure you're reading the right file and saving everything correctly."
        
    def test_filterKeys(self):
        """
        Test Filtering by Key
        """
        assert callable(getattr(ssar,'filterKeys')) , "filterKeys function not defined"
        assert isinstance(ssar.filterKeys(self.pgh,"Eiffel"),list) , "filterKeys should return a list"
        assert ssar.filterKeys(self.pgh,"Eiffel") == [], "This result should be empty list"
        assert ssar.filterKeys(self.pgh,"Towe") == ['U.S. Steel Tower', 'Gulf Tower', 'Tower at PNC Plaza'], "Don't check for an exact match"
    ...
    def test_columSet(self):
        ...
        Test Colum to Set
        """
        assert callable(getattr(ssar,'columnToSet')) , "columnToSet function not defined"
    ...
    def test_filterColumn(self):
        ...
        """
        assert callable(getattr(ssar,'filterByColumn')) , "filterByColumn function not defined"
        assert isinstance(ssar.filterByColumn(self.pgh,0,'center'), list) , "Make sure to return a list"
